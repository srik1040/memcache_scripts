...
class MemcacheTestClient(object):
    """
    """
    def __init__(self, ipAddress='127.0.0.1', port='11211', debug=0, max_retry_count=1):
        self.successCount = 0
        self.failureCount = 0
        self.server = ipAddress + ":" + port
        self.debug_mode = debug
        self.max_retry = max_retry_count
    ...
    def setData(self, data_dict):
        ...
        for key, val in data_dict.items():
            for i in range(self.max_retry):
                if not self.conn.cas(str(key), val):
...
def main():
    ...
    max_retry_count = config.getint('retry-count', 'max_retry')

    print "Memcache test started...........\n"

    # Instantiating class of MemcacheTestClient
    memObj = MemcacheTestClient(ip_addr, ip_port, max_retry_count=max_retry_count)
